...
RAYGUIDEF Function GuiColorPicker(bounds As Rectangle, RLColor As RLColor) As RLColor                                          ' RLColor Picker control (multiple RLColor controls)
RAYGUIDEF Function GuiColorPanel(bounds As Rectangle, RLColor As RLColor) As RLColor                                          ' RLColor Panel control
RAYGUIDEF Function GuiColorBarAlpha(bounds As Rectangle, alpha as single) As Single                                       ' RLColor Bar Alpha control
RAYGUIDEF Function GuiColorBarHue(bounds As Rectangle, value as Single) As Single                                         ' RLColor Bar Hue control
...
RAYGUIDEF Sub GuiDrawIcon( iconId as long, position as Vector2 , pixelSize as long, color0 As RLColor)
...
' Types and Structures Definition
'-----------------------------------------=1
' Gui control property style RLColor element
...
'---------------------------------------=1-
Declare Sub DrawRectangle(x as long, y as long, width0 As long, height0 As long, color0 As RLColor)        ' -=1 GuiDrawRectangle(), GuiDrawIcon()

Declare Sub DrawRectangleGradientEx(rec as Rectangle, RLColor col1, RLColor col2, RLColor col3, RLColor col4)     ' -=1 GuiColorPicker()
Declare Sub DrawTriangle(v1 as Vector2, v2 as Vector2, v3 as Vector2, color0 As RLColor)                              ' -=1 GuiDropdownBox(), GuiScrollBar()
Declare  Sub DrawTextureRec(texture as Texture2D , sourceRec as Rectangle , position as Vector2 , tint As RLColor)       ' -=1 GuiImageButtonEx()

Declare  Sub DrawTextRec(font0 as Font, text As Const zstring Ptr, rec as Rectangle, fontSize as single , spacing as Single, wordWrap as boolean, tint As RLColor) ' -=1 GuiTextBoxMulti()
'---------------------------------------=1-
...
Declare  Sub DrawTextEx(font0 as Font, text As Const zstring Ptr, position as Vector2 , fontSize as Single , spacing as Single, tint As RLColor)  ' -=1 GuiDrawText()

declare function LoadFontEx( fileName As Const zstring Ptr, fontSize as long, fontChars as long ptr, glyphCount as long) As Font ' -=1 GuiLoadStyle()
...
'---------------------------------------=1-
declare function  GetColor(hexValue As long)   As RLColor                ' Returns a RLColor struct from hexadecimal value
declare function ColorToInt(color0 As RLColor)  As long                ' Returns hexadecimal value for a Color
declare function  Fade(color0 As RLColor, alpha0 as Single) As RLColor       ' RLColor fade-in or fade-out, alpha goes from 0.0f to 1.0f
...
Declare  Sub DrawRectangleGradientV(long posX, long posY, width0 As long, height0 As long, color1 As RLColor, color1 As RLColor)  ' Draw rectangle vertical gradient
'---------------------------------------=1-
...
'static
Declare Sub GuiDrawText( text As Const zstring Ptr, bounds As Rectangle, alignment as long, tint As RLColor)         ' Gui draw text using default font
'static
Declare Sub GuiDrawRectangle(rec as Rectangle, borderWidth as long , bordercolour As RLColor , color0 As RLColor)   ' Gui draw rectangle using default raygui style
'static
...
'static
Declare Function ConvertHSVtoRGB(hsv as Vector3)   As  Vector3                ' Convert RLColor data from HSV to RGB
'static
Declare Function ConvertRGBtoHSV(rgb0 as Vector3)  as  Vector3                 ' Convert RLColor data from RGB to HSV
...
Function GuiWindowBox(bounds As Rectangle,  title0 As Const zstring Ptr) As boolean
	...
	Dim As Rectangle statusBar0 = Rectangle( bounds.x, bounds.y, bounds.width_, statusBarHeight )
	If (bounds.height_ < statusBarHeight*2) Then bounds.height_ = statusBarHeight*2

	Dim As Rectangle windowPanel =  Rectangle(bounds.x, bounds.y + statusBarHeight - 1, bounds.width_, bounds.height_ - statusBarHeight )
	Dim As Rectangle closeButtonRec =  Rectangle(statusBar0.x + statusBar0.width_ - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - 20, _
...
Sub GuiGroupBox(bounds As Rectangle, text0 As zstring Ptr)
	...
	' Draw control
	'----------------------------------
	'    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle) bounds.x, bounds.y, GROUPBOX_LINE_THICK, bounds.height_ }, 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, iif(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha))
	GuiDrawRectangle(Rectangle(bounds.x, bounds.y, GROUPBOX_LINE_THICK, bounds.height_ ), 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, iif(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha))
	'    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle) bounds.x, bounds.y + bounds.height_ - 1, bounds.width_, GROUPBOX_LINE_THICK }, 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, IIf(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha))
	GuiDrawRectangle(Rectangle(bounds.x, bounds.y + bounds.height_ - 1, bounds.width_, GROUPBOX_LINE_THICK ), 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, IIf(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha))
	'    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle) bounds.x + bounds.width_ - 1, bounds.y, GROUPBOX_LINE_THICK, bounds.height_ }, 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, iif(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha))
	GuiDrawRectangle(Rectangle(bounds.x + bounds.width_ - 1, bounds.y, GROUPBOX_LINE_THICK, bounds.height_ ), 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, iif(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha))

	GuiLine(Rectangle(bounds.x, bounds.y, bounds.width_, 1 ), text0)
	'----------------------------------
End Sub
...
Sub GuiLine(bounds As Rectangle, text0 As Const zstring Ptr)
	...
	Dim Color0 As RLColor = Fade(GetColor(GuiGetStyle(DEFAULT, iif(state = GUI_STATE_DISABLED, BORDER_COLOR_DISABLED , LINE_COLOR))), guiAlpha)

	' Draw control
	'----------------------------------
	'    if (text = NULL) then GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle) bounds.x, bounds.y + bounds.height_/2, bounds.width_, 1 }, 0, BLANK, color)
	if (text0 = NULL) then
		GuiDrawRectangle(Rectangle(bounds.x, bounds.y + bounds.height_/2, bounds.width_, 1 ), 0, BLANK, Color0)
	Else
		Dim As Rectangle textBounds '=  (0 )
		textBounds.width_ = GetTextWidth(text0)      ' TODO: Consider text icon
		textBounds.height_ = GuiGetStyle(DEFAULT, TEXT_SIZE)
		...
		'GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle) bounds.x + LINE_TEXT_PADDING + textBounds.width_ + 4, bounds.y, bounds.width_ - textBounds.width_ - LINE_TEXT_PADDING - 4, 1 }, 0, BLANK, color)
		GuiDrawRectangle(Rectangle(bounds.x + LINE_TEXT_PADDING + textBounds.width_ + 4, bounds.y, bounds.width_ - textBounds.width_ - LINE_TEXT_PADDING - 4, 1 ), 0, BLANK, Color0)
	EndIf
	'----------------------------------
End sub
...
function GuiScrollPanel(bounds As Rectangle, content as Rectangle, scroll As Vector2 ptr) as Rectangle
	...
	Dim As boolean hasHorizontalScrollBar = IIf(content.width_ > bounds.width_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH), true , FALSE)
	Dim As boolean hasVerticalScrollBar = IIf(content.height_ > bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH), true , FALSE)

	' Recheck to account for the other scrollbar being visible
	if (Not hasHorizontalScrollBar) Then hasHorizontalScrollBar = IIf(hasVerticalScrollBar and (content.width_ > (bounds.width_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH))), true , false)
	if (Not hasVerticalScrollBar) Then hasVerticalScrollBar = IIf(hasHorizontalScrollBar And (content.height_ > (bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH))), true , false)
	...
	' Dim As Const Rectangle horizontalScrollBar =  (single)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (single)bounds.x + verticalScrollBarWidth : (single)bounds.x) + GuiGetStyle(DEFAULT, BORDER_WIDTH), (single)bounds.y + bounds.height_ - horizontalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH), (single)bounds.width_ - verticalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH), (single)horizontalScrollBarWidth }
	' Dim As Const Rectangle verticalScrollBar =  (single)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (single)bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) : (single)bounds.x + bounds.width_ - verticalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH)), (single)bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), (single)verticalScrollBarWidth, (single)bounds.height_ - horizontalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) }
	...
	bounds.y + bounds.height_ - horizontalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH), _
	bounds.width_ - verticalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH), _
	horizontalScrollBarWidth )

	Dim As Const Rectangle verticalScrollBar =   Rectangle(IIf((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) = SCROLLBAR_LEFT_SIDE), bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) , bounds.x + bounds.width_ - verticalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH)), _
	bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), _
	verticalScrollBarWidth, _
	bounds.height_ - horizontalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) )

	' Calculate view area (area without the scrollbars)
	Dim As Rectangle view0 = iif(GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) = SCROLLBAR_LEFT_SIDE , _
	Rectangle(bounds.x + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth ) , _
	Rectangle(bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth ))

	' Clip view area to the actual content size
	if (view0.width_ > content.width_) Then view0.width_ = content.width_
	if (view0.height_ > content.height_) Then view0.height_ = content.height_
	...
	Dim As Const long horizontalMax = IIf(hasHorizontalScrollBar, content.width_ - bounds.width_ + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH) - iif(GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) = SCROLLBAR_LEFT_SIDE, verticalScrollBarWidth , 0) , -GuiGetStyle(DEFAULT, BORDER_WIDTH))
	Dim As Const long verticalMin = IIf(hasVerticalScrollBar, -GuiGetStyle(DEFAULT, BORDER_WIDTH) , -GuiGetStyle(DEFAULT, BORDER_WIDTH))
	Dim As Const long verticalMax = IIf(hasVerticalScrollBar, content.height_ - bounds.height_ + horizontalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH) , -GuiGetStyle(DEFAULT, BORDER_WIDTH))
	...
	if (hasHorizontalScrollBar) Then
		...
		GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, ((bounds.width_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth)/content.width_)*(bounds.width_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth))
	...
	if (hasVerticalScrollBar) Then
		...
		GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, ((bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth)/content.height_)* (bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth))
	...
	if (hasHorizontalScrollBar andalso hasVerticalScrollBar) Then
		...
		Dim As Rectangle corner =  Rectangle(horizontalScrollBar.x + horizontalScrollBar.width_ + 2, verticalScrollBar.y + verticalScrollBar.height_ + 2, horizontalScrollBarWidth - 4, verticalScrollBarWidth - 4 )
...
Function GuiLabelButton(bounds As Rectangle, text0 As Const zstring Ptr) As boolean
	...
	if (bounds.width_ < textWidth) Then bounds.width_ = textWidth
...
Function GuiImageButton(bounds As Rectangle, text As Const zstring Ptr, texture as Texture2D ) As boolean
	...
	return GuiImageButtonEx(bounds, text, texture, Rectangle( 0, 0, texture.width_, texture.height_ ))
End Function
...
Function GuiImageButtonEx(bounds As Rectangle, text0 As Const zstring Ptr, texture as Texture2D , texSource as Rectangle ) As boolean
	...
	if (texture.id > 0) Then DrawTextureRec(texture, texSource, Vector2(bounds.x + bounds.width_/2 - texSource.width_/2, bounds.y + bounds.height_/2 - texSource.height_/2 ), Fade(GetColor(GuiGetStyle(BUTTON, TEXT + (state*3))), guiAlpha))
	'---------------------------------=1
...
Function GuiToggleGroup(bounds As Rectangle, text As Const zstring Ptr, active as long) As long
	...
	for i As long=0 To itemsCount-1'(long i = 0 i < itemsCount i+=1)
		...
		if (prevRow <> rows(i)) Then
			...
			bounds.y += (bounds.height_ + GuiGetStyle(TOGGLE, GROUP_PADDING))
			prevRow = rows(i)
		EndIf
...
Function GuiCheckBox(bounds As Rectangle, text0 As Const zstring Ptr,checked as boolean) As boolean
	...
	if (text0 <> NULL) Then
		...
		textBounds.width_ = GetTextWidth(text0)
		textBounds.height_ = GuiGetStyle(DEFAULT, TEXT_SIZE)
		textBounds.x = bounds.x + bounds.width_ + GuiGetStyle(CHECKBOX, TEXT_PADDING)
		textBounds.y = bounds.y + bounds.height_/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2
		if (GuiGetStyle(CHECKBOX, TEXT_ALIGNMENT) = GUI_TEXT_ALIGN_LEFT) Then textBounds.x = bounds.x - textBounds.width_ - GuiGetStyle(CHECKBOX, TEXT_PADDING)
	EndIf
	...
	if (checked) Then
		...
		bounds.height_ - 2*(GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING)) )
...
Function GuiComboBox(bounds As Rectangle, text0 As Const zstring Ptr, active as long) As long
	...
	bounds.width_ -= (GuiGetStyle(COMBOBOX, COMBO_BUTTON_WIDTH) + GuiGetStyle(COMBOBOX, COMBO_BUTTON_PADDING))

	Dim As Rectangle selector = Rectangle(bounds.x + bounds.width_ + GuiGetStyle(COMBOBOX, COMBO_BUTTON_PADDING), _
	bounds.y, GuiGetStyle(COMBOBOX, COMBO_BUTTON_WIDTH), bounds.height_ )
...
Function GuiDropdownBox(bounds As Rectangle, text0 As Const zstring Ptr, active0 as long ptr, editMode as boolean) As boolean
	...
	boundsOpen.height_ = (itemsCount + 1)*(bounds.height_ + GuiGetStyle(DROPDOWNBOX, DROPDOWN_ITEMS_PADDING))
	...
	if ((state <> GUI_STATE_DISABLED) andalso not guiLocked AndAlso (itemsCount > 1)) Then
		...
		if (editMode) Then
			...
			for i As long=0 To itemsCount-1
				...
				itemBounds.y += (bounds.height_ + GuiGetStyle(DROPDOWNBOX, DROPDOWN_ITEMS_PADDING))
	...
	DrawTriangle(Vector2( bounds.x + bounds.width_ - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height_/2 - 2 ), _
	Vector2( bounds.x + bounds.width_ - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING) + 5, bounds.y + bounds.height_/2 - 2 + 5 ), _
	Vector2( bounds.x + bounds.width_ - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING) + 10, bounds.y + bounds.height_/2 - 2 ),_
	Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))), guiAlpha))

	'GuiDrawText("v", RAYGUI_CLITERAL(Rectangle) bounds.x + bounds.width_ - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height_/2 - 2, 10, 10 },
...
'  	bounds.y + bounds.height_/2 - GuiGetStyle(DEFAULT, TEXT_SIZE), _
'  	1, _
...
'				Dim As long maxWidth = (bounds.width_ - (GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING)*2))
'
...
'				cursor.x = bounds.x + GetTextWidth(text0)/2 + bounds.width_/2 + 1
'			elseif (textAlignment = GUI_TEXT_ALIGN_RIGHT) Then
'				cursor.x = bounds.x + bounds.width_ - GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING)
'			EndIf
'		else
'			if (CheckCollisionPointRec(mousePointeger, bounds)) Then
'				state = GUI_STATE_FOCUSED
...
function GuiTextBox( bounds as Rectangle, text_ as string, textSize as long, editMode as boolean ) as boolean
	...
	if( ( state <> GUI_STATE_DISABLED ) andAlso not guiLocked ) then
		...
		if( editMode ) then
			...
			if( textAlignment = GUI_TEXT_ALIGN_CENTER ) then
				cursor.x = bounds.x + GetTextWidth( text_ ) / 2 + bounds.width_ / 2 + 1
			elseif( textAlignment = GUI_TEXT_ALIGN_RIGHT ) then
				cursor.x = bounds.x + bounds.width_ - GuiGetStyle( TEXTBOX, TEXT_INNER_PADDING )
			endif
		else
...
Function GuiSpinner(bounds As Rectangle, text0 As Const zstring Ptr, value as long Ptr, minValue as long, maxValue as long, editMode as boolean) As boolean
	...
	bounds.width_ - 2*(GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_PADDING)), bounds.height_ )
	Dim As Rectangle leftButtonBound =  Rectangle(bounds.x, bounds.y, GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), bounds.height_ )
	'Dim As Rectangle rightButtonBound =  Rectangle(bounds.x + bounds.width_ - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), bounds.y, GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), bounds.height_ )
	'hack
	Dim As Rectangle rightButtonBound =  Rectangle(bounds.x + bounds.width_ - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), bounds.y, GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), bounds.height_ )
	...
	if (text0 <> NULL) Then
		...
		textBounds.width_ = GetTextWidth(text0)
		textBounds.height_ = GuiGetStyle(DEFAULT, TEXT_SIZE)
		textBounds.x = bounds.x + bounds.width_ + GuiGetStyle(SPINNER, TEXT_PADDING)
		textBounds.y = bounds.y + bounds.height_/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2
		if (GuiGetStyle(SPINNER, TEXT_ALIGNMENT) = GUI_TEXT_ALIGN_LEFT) Then textBounds.x = bounds.x - textBounds.width_ - GuiGetStyle(SPINNER, TEXT_PADDING)
	EndIf
...
'		textBounds.y = bounds.y + bounds.height_/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2
'		if (GuiGetStyle(VALUEBOX, TEXT_ALIGNMENT) = GUI_TEXT_ALIGN_LEFT) Then textBounds.x = bounds.x - textBounds.width_ - GuiGetStyle(VALUEBOX, TEXT_PADDING)
'	EndIf
'
'	' Update control
'	'----------------------------------
...
'
'				Dim As long maxWidth = bounds.width_
...
'		Dim As Rectangle cursor =  Rectangle(bounds.x + GetTextWidth(textValue)/2 + bounds.width_/2 + 2, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 1, bounds.height_ - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) )
...
function GuiTextBoxMulti( bounds as Rectangle, text_ as string, textSize as long, editMode as boolean ) as boolean
	...
	bounds.width_ - 2 * GuiGetStyle( TEXTBOX, TEXT_INNER_PADDING ), _
	bounds.height_ - 2 * GuiGetStyle( TEXTBOX, TEXT_INNER_PADDING ) )
	...
	if( ( state <> GUI_STATE_DISABLED ) andAlso not guiLocked ) then
		...
		if( editMode ) then
			...
			if( textSize.y < ( textAreaBounds.height_ - GuiGetStyle( DEFAULT, TEXT_SIZE ) ) ) then
			...
			do while( i < keyCount andAlso currentLine < keyCount )
				...
				if( text_[ i ] = asc( !"\n" ) orElse textWidth >= textAreaBounds.width_ ) then
					currentLine += 1
					textWidth = 0
	...
	' no existe ya en la 5.0, y no hay sustituto, lo camibio por:
   Dim As Vector2 positiontext
   positiontext.x=textAreaBounds.x
   positiontext.y=textAreaBounds.y
   DrawTextEx(guiFont , text_ , positiontext , GuiGetStyle(DEFAULT, TEXT_SIZE) , GuiGetStyle(DEFAULT, TEXT_SPACING) , Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state*3))), guiAlpha) ) 
...
Function GuiSliderPro(bounds As Rectangle, textLeft As Const zstring Ptr, textRight As Const zstring Ptr, value as Single, minValue as Single, maxValue as Single, sliderWidth As long ) As Single
	...
	if ((state <> GUI_STATE_DISABLED) andalso not guiLocked) Then
		...
		if (CheckCollisionPointRec(mousePointeger, bounds)) Then
			...
			if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) Then
				...
				value = ((maxValue - minValue)*(mousePointeger.x - (bounds.x + sliderWidth/2)))/(bounds.width_ - sliderWidth) + minValue

				if (sliderWidth > 0) Then
					slider0.x = mousePointeger.x - slider0.width_/2  ' Slider

				ElseIf (sliderWidth = 0) Then
					slider0.width_ = sliderValue          ' SliderBar
				EndIf
			else
				state = GUI_STATE_FOCUSED
			EndIf
	...
	if (sliderWidth > 0) Then       ' Slider
		...
		elseif ((slider0.x + slider0.width_) >= (bounds.x + bounds.width_)) Then
			slider0.x = bounds.x + bounds.width_ - slider0.width_ - GuiGetStyle(SLIDER, BORDER_WIDTH)
		EndIf

	elseif (sliderWidth = 0) Then ' SliderBar

		if (slider0.width_ > bounds.width_) Then slider0.width_ = bounds.width_ - 2*GuiGetStyle(SLIDER, BORDER_WIDTH)
	EndIf
	'----------------------------------
	...
	if (textLeft <> NULL) Then
		...
		Dim As Rectangle textBounds
		textBounds.width_ = GetTextWidth(textLeft)  ' TODO: Consider text icon
		textBounds.height_ = GuiGetStyle(DEFAULT, TEXT_SIZE)
		textBounds.x = bounds.x - textBounds.width_ - GuiGetStyle(SLIDER, TEXT_PADDING)
		textBounds.y = bounds.y + bounds.height_/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2
	...
	if (textRight <> NULL) Then
		...
		Dim As Rectangle textBounds
		textBounds.width_ = GetTextWidth(textRight)  ' TODO: Consider text icon
		textBounds.height_ = GuiGetStyle(DEFAULT, TEXT_SIZE)
		textBounds.x = bounds.x + bounds.width_ + GuiGetStyle(SLIDER, TEXT_PADDING)
		textBounds.y = bounds.y + bounds.height_/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2
...
Function GuiProgressBar(bounds As Rectangle, textLeft As Const zstring Ptr, textRight As Const zstring Ptr, value as Single, minValue as Single, maxValue as single) As Single
	...
	bounds.height_ - 2*GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - 2*GuiGetStyle(PROGRESSBAR, PROGRESS_PADDING) )

	' Update control
	'----------------------------------
	if (state <> GUI_STATE_DISABLED) Then progress.width_ = (value/(maxValue - minValue)*(bounds.width_ - 2*GuiGetStyle(PROGRESSBAR, BORDER_WIDTH)))
	'----------------------------------
...
Function GuiScrollBar(bounds As Rectangle, value as long, minValue as long, maxValue as long) As long
	...
	Dim As boolean isVertical = iif(bounds.width_ > bounds.height_, false , TRUE)

	' The size (width or height depending on scrollbar type) of the spinner buttons
	Dim As Const long spinnerSize = IIf(GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE), iif(isVertical, bounds.width_ - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) , bounds.height_ - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH)) , 0)
	...
	if (isVertical) Then
		...
		arrowDownRight = Rectangle( bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), bounds.y + bounds.height_ - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), spinnerSize, spinnerSize)
		scrollbar0 = Rectangle( bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), arrowUpLeft.y + arrowUpLeft.height_, bounds.width_ - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)), bounds.height_ - arrowUpLeft.height_ - arrowDownRight.height_ - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) )
		sliderSize = iif(sliderSize >= scrollbar0.height_, (scrollbar0.height_ - 2) , sliderSize )    ' Make sure the slider won't get outside of the scrollbar
		slider0 = Rectangle( bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING), scrollbar0.y + (((value - minValue)/range)*(scrollbar0.height_ - sliderSize)), bounds.width_ - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING)), sliderSize )

	else

		arrowDownRight = Rectangle( bounds.x + bounds.width_ - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), spinnerSize, spinnerSize)
		scrollbar0 = Rectangle( arrowUpLeft.x + arrowUpLeft.width_, bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), bounds.width_ - arrowUpLeft.width_ - arrowDownRight.width_ - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH), bounds.height_ - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)))
		sliderSize = iif(sliderSize >= scrollbar0.width_, (scrollbar0.width_ - 2) , sliderSize)       ' Make sure the slider won't get outside of the scrollbar
		slider0 = Rectangle( scrollbar0.x + (((value - minValue)/range)*(scrollbar0.width_ - sliderSize)), bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING), sliderSize, bounds.height_ - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING)) )
	endif
		...
		if (CheckCollisionPointRec(mousePointeger, bounds)) then
			...
			elseif (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) Then
				...
				if (Not isVertical)Then
					...
					Dim As Rectangle scrollArea =  Rectangle (arrowUpLeft.x + arrowUpLeft.width_, arrowUpLeft.y, scrollbar0.width_, bounds.height_ - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH))
					if (CheckCollisionPointRec(mousePointeger, scrollArea)) Then value = ((mousePointeger.x - scrollArea.x - slider0.width_/2)*range)/(scrollArea.width_ - slider0.width_) + minValue

				else

					Dim As Rectangle scrollArea =  Rectangle(arrowUpLeft.x, arrowUpLeft.y+arrowUpLeft.height_, bounds.width_ - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH),  scrollbar0.height_)
					if (CheckCollisionPointRec(mousePointeger, scrollArea)) Then value = ((mousePointeger.y - scrollArea.y - slider0.height_/2)*range)/(scrollArea.height_ - slider0.height_) + minValue
				EndIf
			EndIf
		endif
	...
	Dim As RLColor lineColor = Fade(GetColor(GuiGetStyle(BUTTON, TEXT + state*3)), guiAlpha)
...
Function GuiListViewEx overload(bounds As Rectangle, text0 As Const zstring Ptr Ptr, count as long , focus as long ptr, scrollIndex as long ptr, active as long) As long
	...
	if ((GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) + GuiGetStyle(LISTVIEW, LIST_ITEMS_PADDING))*count > bounds.height_) Then useScrollBar = true
	...
	itemBounds.width_ = bounds.width_ - 2*GuiGetStyle(LISTVIEW, LIST_ITEMS_PADDING) - GuiGetStyle(DEFAULT, BORDER_WIDTH)
	itemBounds.height_ = GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT)
	if (useScrollBar) Then itemBounds.width_ -= GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH)

	' Get items on the list
	Dim As long visibleItems = bounds.height_/(GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) + GuiGetStyle(LISTVIEW, LIST_ITEMS_PADDING))
	...
	if (useScrollBar) Then
		...
		bounds.height_ - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) _
		)

		' Calculate percentage of visible items and apply same percentage to scrollbar
		Dim As Single percentVisible = (endIndex - startIndex)/count
		Dim As Single sliderSize = bounds.height_*percentVisible
...
Function GuiColorPanelEx(bounds As Rectangle, color0 As RLColor, hue As Single) As RLColor
	...
	Dim hsv as Vector3 = ConvertRGBtoHSV(vcolor)
		...
		pickerSelector.y = bounds.y + (1.0f - hsv.z)*bounds.height_  ' HSV: Value

	Dim As Vector3 maxHue =  Vector3(iif(hue >= 0.0f , hue , hsv.x), 1.0f, 1.0f)
	Dim As Vector3 rgbHue = ConvertHSVtoRGB(maxHue)
	Dim As RLColor maxHueCol =  RLColor((255.0f*rgbHue.x),_
	(255.0f*rgbHue.y),_
	(255.0f*rgbHue.z), 255 )

	Dim As Const RLColor colWhite = RLColor( 255, 255, 255, 255 )
	Dim As Const RLColor colBlack = RLColor( 0, 0, 0, 255 )
	...
	if ((state <> GUI_STATE_DISABLED) andalso not guiLocked) Then
		...
		if (CheckCollisionPointRec(mousePointer, bounds)) Then
			...
			if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) Then
				...
				colorPick.y /= bounds.height_    ' Get normalized value on y
				...
				' NOTE: Vector3ToRLColor() only available on raylib 1.8.1
				color0 = RLColor( (255.0f*rgb1.x),_
				(255.0f*rgb1.y),_
				(255.0f*rgb1.z),_
...
function GuiColorPanel(bounds As Rectangle, color0 As RLColor) As RLColor
...
Function GuiColorBarAlpha(bounds As Rectangle, alpha0 as Single) As single
	...
	Dim As GuiControlState state = guiState
	Dim As Rectangle selector = Rectangle(bounds.x + alpha0*bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), bounds.height_ + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2 )
	...
	if ((state <> GUI_STATE_DISABLED) andalso not guiLocked) Then
		...
		if (CheckCollisionPointRec(mousePointeger, bounds) OrElse _
			...
			if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) Then
				...
				state = GUI_STATE_PRESSED
				selector.x = mousePointeger.x - selector.width_/2

				alpha0 = (mousePointeger.x - bounds.x)/bounds.width_
				if (alpha0 <= 0.0f) Then alpha0 = 0.0f
				if (alpha0 >= 1.0f) Then alpha0 = 1.0f
				'selector.x = bounds.x + (((alpha0 - 0)/(100 - 0))*(bounds.width_ - 2*GuiGetStyle(SLIDER, BORDER_WIDTH))) - selector.width_/2
	...
	if (state <> GUI_STATE_DISABLED) Then
		...
		Dim As long checksX = bounds.width_/COLORBARALPHA_CHECKED_SIZE
		Dim As long checksY = bounds.height_/COLORBARALPHA_CHECKED_SIZE
		...
		DrawRectangleGradientEx(bounds, RLColor( 255, 255, 255, 0 ), RLColor(255, 255, 255, 0 ), Fade(RLColor( 0, 0, 0, 255 ), guiAlpha), Fade(RLColor(0, 0, 0, 255 ), guiAlpha))
...
Function GuiColorBarHue(bounds As Rectangle, hue As single) As single
	...
	Dim As GuiControlState state = guiState
	Dim As Rectangle selector = Rectangle(bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.y + hue/360.0f*bounds.height_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.width_ + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2, GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) )
	...
	if ((state <> GUI_STATE_DISABLED) andalso not guiLocked) Then
		...
		if (CheckCollisionPointRec(mousePointer, bounds) OrElse _
			...
			if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) Then
				...
				state = GUI_STATE_PRESSED
				selector.y = mousePointer.y - (selector.height_/2)

				hue = (mousePointer.y - bounds.y)*(360.0f/bounds.height_)
	...
	if (state <> GUI_STATE_DISABLED) Then
		...
		DrawRectangleGradientV(bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.y + 0*(bounds.height_/6) + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.height_/6, Fade(RLColor( 255,0,0,255 ), guiAlpha), Fade(RLColor( 255,255,0,255 ), guiAlpha))
		DrawRectangleGradientV(bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.y + 1*(bounds.height_/6) + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.height_/6, Fade(RLColor( 255,255,0,255 ), guiAlpha), Fade(RLColor(0,255,0,255 ), guiAlpha))
		DrawRectangleGradientV(bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.y + 2*(bounds.height_/6) + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2-1, bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.height_/6+1, Fade(RLColor( 0,255,0,255 ), guiAlpha), Fade(RLColor( 0,255,255,255 ), guiAlpha))
		DrawRectangleGradientV(bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.y + 3*(bounds.height_/6) + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2-1, bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.height_/6+1, Fade(RLColor( 0,255,255,255 ), guiAlpha), Fade(RLColor( 0,0,255,255 ), guiAlpha))
		DrawRectangleGradientV(bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.y + 4*(bounds.height_/6) + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.height_/6, Fade(RLColor( 0,0,255,255 ), guiAlpha), Fade(RLColor( 255,0,255,255 ), guiAlpha))
		DrawRectangleGradientV(bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2, bounds.y + 5*(bounds.height_/6) + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)/2-1, bounds.width_ - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), bounds.height_/6+1 - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), Fade(RLColor( 255,0,255,255 ), guiAlpha), Fade(RLColor( 255,0,0,255 ), guiAlpha))

	else
		DrawRectangleGradientV(bounds.x, bounds.y, bounds.width_, bounds.height_, Fade(Fade(GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), guiAlpha), Fade(GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), guiAlpha))
	EndIf
	GuiDrawRectangle(bounds, 1, Fade(GetColor(GuiGetStyle(COLORPICKER, BORDER + state*3)), guiAlpha), BLANK)
...
' TODO: RLColor GuiColorBarSat() [WHITE->color]
' TODO: RLColor GuiColorBarValue() [BLACK->color], HSV / HSL
' TODO: single GuiColorBarLuminance() [BLACK->WHITE]

' RLColor Picker control
' NOTE: It's divided in multiple controls:
'      RLColor GuiColorPanel(bounds As Rectangle, RLColor As RLColor)
...
Function GuiColorPicker(bounds As Rectangle, color0 As RLColor) As RLColor

	color0 = GuiColorPanel(bounds, color0)

	Dim As Rectangle boundsHue =  Rectangle(bounds.x + bounds.width_ + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), bounds.y, GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), bounds.height_ )
	dim As Rectangle boundsAlpha =  Rectangle(bounds.x, bounds.y + bounds.height_ + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), bounds.width_, GuiGetStyle(COLORPICKER, HUEBAR_WIDTH) )
	...
	color0 = RLColor( roundf(rgb0.x*255.0f), roundf(rgb0.y*255.0f), roundf(rgb0.z*255.0f), color0.a )
	'-----------------------------------------------
	#define COLORBARALPHA_CHECKED_SIZE   10

	Dim As Rectangle colB=Rectangle(boundsAlpha.x+boundsAlpha.width_+ GuiGetStyle(COLORPICKER, HUEBAR_PADDING),boundsAlpha.y,boundsAlpha.height_,boundsAlpha.height_)
	Dim As GuiControlState state = guiState

	if (state <> GUI_STATE_DISABLED) Then

		Dim As long checksX = colB.width_/COLORBARALPHA_CHECKED_SIZE
		Dim As long checksY = colB.height_/COLORBARALPHA_CHECKED_SIZE
		Dim c0 As RLColor=WHITE
		Dim c1 As RLColor=BLACK
		Dim c2 As RLColor
...
Function GuiMessageBox(bounds As Rectangle,  title As Const zstring Ptr,  message As Const zstring Ptr,  buttons As Const zstring Ptr) As long
	...
	buttonBounds.y = bounds.y + bounds.height_ - MESSAGEBOX_BUTTON_HEIGHT - MESSAGEBOX_BUTTON_PADDING
	buttonBounds.width_ = (bounds.width_ - MESSAGEBOX_BUTTON_PADDING*(buttonsCount + 1))/buttonsCount
	buttonBounds.height_ = MESSAGEBOX_BUTTON_HEIGHT

	Dim As Vector2 textSize = MeasureTextEx(guiFont, message, GuiGetStyle(DEFAULT, TEXT_SIZE), 1)

	Dim As Rectangle textBounds
	textBounds.x = bounds.x + bounds.width_/2 - textSize.x/2
	textBounds.y = bounds.y + WINDOW_STATUSBAR_HEIGHT + (bounds.height_ - WINDOW_STATUSBAR_HEIGHT - MESSAGEBOX_BUTTON_HEIGHT - MESSAGEBOX_BUTTON_PADDING)/2 - textSize.y/2
	textBounds.width_ = textSize.x
	textBounds.height_ = textSize.y
	...
	for i As long  = 0 to buttonsCount -1
		...
		buttonBounds.x += (buttonBounds.width_ + MESSAGEBOX_BUTTON_PADDING)
	Next
...
Function GuiTextInputBox(bounds As Rectangle,  title As Const zstring Ptr,  message As Const zstring Ptr,  buttons As Const zstring Ptr, text as zstring Ptr) As long
	...
	buttonBounds.y = bounds.y + bounds.height_ - TEXTINPUTBOX_BUTTON_HEIGHT - TEXTINPUTBOX_BUTTON_PADDING
	buttonBounds.width_ = (bounds.width_ - TEXTINPUTBOX_BUTTON_PADDING*(buttonsCount + 1))/buttonsCount
	buttonBounds.height_ = TEXTINPUTBOX_BUTTON_HEIGHT

	Dim As long messageInputHeight = bounds.height_ - WINDOW_STATUSBAR_HEIGHT - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - TEXTINPUTBOX_BUTTON_HEIGHT - 2*TEXTINPUTBOX_BUTTON_PADDING
	...
	EndIf
	textBoxBounds.width_ = bounds.width_ - TEXTINPUTBOX_BUTTON_PADDING*2
	textBoxBounds.height_ = TEXTINPUTBOX_HEIGHT
...
Function GuiGrid(bounds As Rectangle, spacing as Single, subdivs as long) As Vector2
	...
	Dim As long linesV = ((bounds.width_/spacing))*subdivs + 1
	Dim As long linesH = ((bounds.height_/spacing))*subdivs + 1
		...
		case GUI_STATE_NORMAL:
			...
			if (subdivs > 0) Then
				...
				for i As long  = 0 to linesV-1
					...
					Dim As Rectangle lineV =  Rectangle(bounds.x + spacing * i / subdivs, bounds.y, 1, bounds.height_ )
				...
				for i As long = 0 To linesH-1
					...
					dim As Rectangle lineH =  Rectangle(bounds.x, bounds.y + spacing * i / subdivs, bounds.width_, 1 )
				...
				Select Case As Const (buffer[0])
					...
					Case Asc("f")
						...
						if (charmapFileName[0] <> Asc("0")) then
							...
							Dim As zstring ptr charValues = LoadFileText(charmapFileName)
							if (charValues <> NULL) Then

								Dim glyphCount as long = 0
								Dim As Const ZString Ptr Ptr chars = TextSplit(charValues, Asc(!"\n"), @glyphCount)

								Dim as long Ptr values = Cast(long Ptr,RAYGUI_MALLOC(glyphCount*sizeof(long)))
								for i As Integer = 0 to glyphCount-1
									values[i] = TextToInteger(chars[i])
								Next

								font0 = LoadFontEx(TextFormat("%s/%s", GetDirectoryPath(fileName), fontFileName), fontSize, values, glyphCount)
						...
						EndIf
						if ((font0.texture.id > 0) AndAlso (font0.glyphCount > 0)) Then GuiSetFont(font0)
						Exit select
	...
	if (tryBinary) Then
		...
		if ((signature[0] = Asc("r")) AndAlso _
			...
			if (fontDataSize > 0) Then
				...
				fread(@font0.glyphCount, 1, sizeof(integer), rgsFile)
				...
				if (fontImageSize > 0) then
					...
					Dim As Image imFont
					imFont.mipmaps = 1
					fread(@imFont.width_, 1, sizeof(integer), rgsFile)
					fread(@imFont.height_, 1, sizeof(integer), rgsFile)
					fread(@imFont.format_, 1, sizeof(integer), rgsFile)

					imFont.data_ = Cast(zstring Ptr,RAYGUI_MALLOC(fontImageSize))
					fread(imFont.data_, 1, fontImageSize, rgsFile)
				...
				' Load font recs data
				font0.recs = Cast(Rectangle Ptr,RAYGUI_CALLOC(font0.glyphCount, sizeof(Rectangle)))
				for i As integer = 0 to font0.glyphCount-1
					fread(@font0.recs[i], 1, sizeof(Rectangle), rgsFile)
				Next

				' Load font chars info data
				font0.glyphs = cast(GlyphInfo Ptr,RAYGUI_CALLOC(font0.glyphCount, sizeof(GlyphInfo)))
				for i As Integer = 0 to font0.glyphCount-1

					fread(@font0.glyphs[i].value, 1, sizeof(integer), rgsFile)
					fread(@font0.glyphs[i].offsetX, 1, sizeof(integer), rgsFile)
					fread(@font0.glyphs[i].offsetY, 1, sizeof(integer), rgsFile)
					fread(@font0.glyphs[i].advanceX, 1, sizeof(integer), rgsFile)
				Next
				...
				if ((whiteRec.width_ <> 0) AndAlso (whiteRec.height_ <> 0)) Then SetShapesTexture(font0.texture, whiteRec)
			EndIf
			#endif
		EndIf
...
Sub GuiDrawIcon( iconId as long, position as Vector2 , pixelSize as long, color0 As RLColor)
...
Function GetTextBounds(control0 as long, bounds As Rectangle)As Rectangle
	...
	textBounds.width_ = bounds.width_ - 2*GuiGetStyle(control0, BORDER_WIDTH)
	textBounds.height_ = bounds.height_ - 2*GuiGetStyle(control0, BORDER_WIDTH)

	' Consider TEXT_PADDING properly, depends on control type and TEXT_ALIGNMENT
	Select Case (control0)

		case COMBOBOX:
			bounds.width_ -= (GuiGetStyle(control0, COMBO_BUTTON_WIDTH) + GuiGetStyle(control0, COMBO_BUTTON_PADDING))
...
Sub GuiDrawText( text0 As Const zstring Ptr, bounds As Rectangle, alignment as long, tint As RLColor)
	...
	if ((text0 <> NULL) AndAlso (text0[0] <> asc(!"\0"))) Then
		...
		Select Case (alignment)
			...
			case GUI_TEXT_ALIGN_LEFT:
				...
				position.x = bounds.x
				position.y = bounds.y + bounds.height_/2 - textHeight/2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height_)

			case GUI_TEXT_ALIGN_CENTER:

				position.x = bounds.x + bounds.width_/2 - textWidth\2 - TEXT_VALIGN_PIXEL_OFFSET(bounds.width_)
				position.y = bounds.y + bounds.height_/2 - textHeight/2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height_)

			case GUI_TEXT_ALIGN_RIGHT:

				position.x = bounds.x + bounds.width_ - textWidth
				position.y = bounds.y + bounds.height_/2 - textHeight/2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height_)
		...
		if (iconId >= 0) Then
			While iconId>=0
				...
				GuiDrawIcon(iconId, Vector2( position.x, bounds.y + bounds.height_/2 - RICON_SIZE/2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height_) ), 1, tint)
				'hack
				'GuiDrawIcon(iconId, Vector2( position.x-(RICON_SIZE/2 ), bounds.y + bounds.height_/2 - RICON_SIZE/2 + TEXT_VALIGN_PIXEL_OFFSET(bounds.height_) ), 1, tint)
...
Sub GuiDrawRectangle(rec as Rectangle, borderWidth as long , borderColor As RLColor , color0 As RLColor)

	if (color0.a > 0) Then

		' Draw rectangle filled with color
		DrawRectangle(rec.x, rec.y, rec.width_, rec.height_, color0)
	EndIf

	if (borderWidth > 0) Then

		' Draw rectangle border lines with color
		DrawRectangle(rec.x, rec.y, rec.width_, borderWidth, borderColor)
		DrawRectangle(rec.x, rec.y + borderWidth, borderWidth, rec.height_ - 2*borderWidth, borderColor)
		DrawRectangle(rec.x + rec.width_ - borderWidth, rec.y + borderWidth, borderWidth, rec.height_ - 2*borderWidth, borderColor)
		DrawRectangle(rec.x, rec.y + rec.height_ - borderWidth, rec.width_, borderWidth, borderColor)
	EndIf
...
' Convert RLColor data from RGB to HSV
...
' Convert RLColor data from HSV to RGB
' NOTE: RLColor data should be passed normalized
...
Function GetColor(hexValue As long) As RLColor

	Dim color0 As RLColor
...
Function ColorToInt(color0 As RLColor) As long
...
Function CheckCollisionPointRec(point0 As Vector2 , rec as Rectangle) As boolean
	...
	if ((point0.x >= rec.x) andalso (point0.x <= (rec.x + rec.width_)) AndAlso _
	(point0.y >= rec.y) AndAlso (point0.y <= (rec.y + rec.height_))) Then collision = true

	return collision
End Function

' RLColor fade-in or fade-out, alpha goes from 0.0f to 1.0f
Function Fade(color0 As RLColor, alpha0 as Single) As RLColor
	...
	elseif (alpha0 > 1.0f) Then
		alpha0 = 1.0f
	EndIf
	Dim As RLColor result =  RLColor(color0.r, color0.g, color0.b, int(255.0f*alpha) )
...
sub DrawRectangleGradientV(posX As long,posY long, width0 As long,height As long, color1 As RLColor, color2 As RLColor)
