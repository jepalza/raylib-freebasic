...
dim as BOOLEAN importWindowActive = false

dim as long widthValue = 0
dim as BOOLEAN widthEditMode = false
dim as long heightValue = 0
dim as BOOLEAN heightEditMode = false
...
dim as BOOLEAN headerSizeEditMode = false
...
dim as BOOLEAN btnLoadPressed = false
...
do while( not WindowShouldClose() )
  ...
  if( IsFileDropped() ) then
    ...
    if( ( droppedFiles.Count = 1 ) andAlso IsFileExtension( droppedFiles.paths[ 0 ], ".raw" ) ) then
      dim as FILE ptr imageFile = fopen( droppedFiles.paths[ 0 ], "rb" )
      ...
      fileNamePath = *droppedFiles.paths[ 0 ] 
  ...
  if( btnLoadPressed ) then
    ...
    if( ( widthValue <> 0 ) andAlso ( heightValue <> 0 ) ) then
      ...
      if( pixelFormatActive = 0 ) then
        ...
        if( bpp = 8 ) then
          if( channels = 1 ) then format_ = PIXELFORMAT_UNCOMPRESSED_GRAYSCALE
          elseif( channels = 2 ) then format_ = PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA
          elseif( channels = 3 ) then format_ = PIXELFORMAT_UNCOMPRESSED_R8G8B8
          elseif( channels = 4 ) then format_ = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8
        elseif( bpp = 32 ) then
          if( channels = 1 ) then format_ = PIXELFORMAT_UNCOMPRESSED_R32
          elseif( channels = 2 ) then TraceLog( LOG_WARNING, "Channel bit-depth not supported!" )
          elseif( channels = 3 ) then format_ = PIXELFORMAT_UNCOMPRESSED_R32G32B32
          elseif( channels = 4 ) then format_ = PIXELFORMAT_UNCOMPRESSED_R32G32B32A32
        elseif( bpp = 16 ) then
  ...
  BeginDrawing()
    ...
    if( imageLoaded ) then 
      DrawTextureEx( texture, Vector2( screenWidth / 2 - texture.width_ * imageScale / 2, screenHeight / 2 - texture.height_ * imageScale / 2 ), 0, imageScale, WHITE )
