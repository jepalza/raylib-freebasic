...
dim as RLColor colours( 0 to 9 )

for i as integer = 0 to 9
  colours( i ) = RLColor( GetRandomValue( 100, 250 ), GetRandomValue( 50, 150 ), GetRandomValue( 10, 100 ), 255 )
next
...
do while( WindowShouldClose()=0)
  ...
  BeginDrawing()
    ...
    BeginTextureMode( target )
      ...
      for i as integer = 0 to 9
        DrawRectangle( 0, ( gameScreenHeight / 10 ) * i, gameScreenWidth, gameScreenHeight / 10, colours( i ) )
      next
      
      DrawText( !"If executed inside a window,\nyou can resize the window,\nand see the screen scaling!", 10, 25, 20, WHITE )
      
      DrawText( TextFormat( "Default Mouse: [%i , %i]", clng( mouse.x ), clng( mouse.y ) ), 350, 25, 20, GREEN )
      DrawText( TextFormat( "Virtual Mouse: [%i , %i]", clng( virtualMouse.x ), clng( virtualMouse.y ) ), 350, 55, 20, YELLOW )
    EndTextureMode()
    
    '' Draw RenderTexture2D to window, properly scaled
    DrawTexturePro(_
      target.texture, Rectangle( 0.0f, 0.0f, target.texture.width_, -target.texture.height_ ), _
