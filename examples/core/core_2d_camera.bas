...
dim as RLColor buildRLColor( 0 to MAX_BUILDINGS - 1 )
...
for i as integer = 0 to MAX_BUILDINGS - 1
  with buildings( i )
    ...
    .y = screenHeight - 130 - .height_
    .x = -6000 + spacing
    
    spacing += .width_
  end with
  
  buildRLColor( i ) = RLColor( GetRandomValue( 200, 240 ), GetRandomValue( 200, 240 ), GetRandomValue( 200, 250 ), 255 )
next
...
do while( not WindowShouldClose() )
  ...
  BeginDrawing()
    ...
    BeginMode2D( camera )
      ...
      for i as integer = 0 to MAX_BUILDINGS - 1
        DrawRectangleRec( buildings( i ), buildRLColor( i ) )
      next
