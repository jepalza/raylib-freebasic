...
dim as RLColor colours( ... ) = { MAROON, ORANGE, DARKGREEN, DARKBLUE, DARKPURPLE, LIME, GOLD, RED }
...
do while(WindowShouldClose()=0)
  ...
  BeginDrawing()
    ...
    for i as integer = 0 to MAX_FONTS - 1
      DrawTextEx( fonts( i ), messages( i ), positions( i ), fonts( i ).baseSize * 2, spacings( i ), colours( i ) )
    next
  EndDrawing()
loop
