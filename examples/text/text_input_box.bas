...
do while(WindowShouldClose()=0)
  ...
  Else 'If( GetMouseCursor() <> MOUSE_CURSOR_DEFAULT ) then
  ...
  BeginDrawing()
    ...
    if( mouseOnText ) then
      DrawRectangleLines( textBox.x, textBox.y, textBox.width_, textBox.height_, RED )
    else
      DrawRectangleLines( textBox.x, textBox.y, textBox.width_, textBox.height_, DARKGRAY )
    end if
