...
do while(WindowShouldClose()=0)
  ...
  if( textureReload ) then
    ...
    select case as const( currentProcess )
      ...
      case COLOR_TINT : ImageColorTint( @image, GREEN )
    ...
    dim as RLColor ptr pixels = LoadImageColors( image ) '' Get pixel data from image (RGBA 32bit)
  ...
  BeginDrawing()
    ...
    for i as integer = 0 to NUM_PROCESSES - 1
      ...
      DrawRectangleLines( toggleRecs( i ).x, toggleRecs( i ).y, toggleRecs( i ).width_, toggleRecs( i ).height_, iif( ( ( i = currentProcess) orElse ( i = mouseHoverRec ) ), BLUE, GRAY ) )
      DrawText( processText( i ), clng( toggleRecs( i ).x + toggleRecs( i ).width_ / 2 - MeasureText( processText( i ), 10 ) / 2 ), clng( toggleRecs( i ).y + 11 ), 10, iif( ( ( i = currentProcess ) orElse ( i = mouseHoverRec ) ), DARKBLUE, DARKGRAY ) )
    next
    
    DrawTexture( texture, screenWidth - texture.width_ - 60, screenHeight / 2 - texture.height_ / 2, WHITE )
    DrawRectangleLines( screenWidth - texture.width_ - 60, screenHeight / 2 - texture.height_ / 2, texture.width_, texture.height_, BLACK )
  EndDrawing()
loop
