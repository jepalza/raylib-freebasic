...
ImageDrawTextEx( @parrots, font, "[Parrots font drawing]", Vector2( 20.0f, 20.0f ), font.baseSize, 0.0f, RED )

dim as Texture2D texture = LoadTextureFromImage( parrots )
UnloadImage( parrots )

var position = Vector2( ( screenWidth / 2 - texture.width_ / 2 ), ( screenHeight / 2 - texture.height_ / 2 - 20 ) )
...
do while(WindowShouldClose()=0)
  ...
  BeginDrawing()
    ...
    if( not showFont ) then
      ...
      DrawTexture( font.texture, screenWidth / 2 - font.texture.width_ / 2, 50, BLACK )
    end if
