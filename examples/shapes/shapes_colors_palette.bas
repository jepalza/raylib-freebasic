/'*******************************************************************************************
*
*   raylib [shapes] example - RLColor palette
*
...
do while(WindowShouldClose()=0)
  ...
  BeginDrawing()
    ...
    for i as integer = 0 to MAX_COLORS_COUNT - 1
      DrawRectangleRec( colorsRecs( i ), Fade( colours( i ), iif( colorState( i ), 0.6f,  1.0f ) ) )
      
      if( IsKeyDown( KEY_SPACE ) orElse colorState( i ) = 1 ) then
        DrawRectangle( colorsRecs( i ).x, colorsRecs( i ).y + colorsRecs( i ).height_ - 26, colorsRecs( i ).width_, 20, BLACK )
        DrawRectangleLinesEx( colorsRecs( i ), 6, Fade( BLACK, 0.3f ) )
        DrawText( colorNames( i ), colorsRecs( i ).x + colorsRecs( i ).width_ - MeasureText( colorNames( i ), 10 ) - 12, _
                  colorsRecs( i ).y + colorsRecs( i ).height_ - 20, 10, colours( i ) )
      end if
    next
  EndDrawing()
loop
